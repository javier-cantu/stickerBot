// sb026_salioError.js
// me salio un error y creo que era por mensajes que no se habian cargado en el cel
// Tambien cambie algunas cosas en la function manejarBotMenu(message)
// Porque creo que luego de poner /bot cualquier usuario podia usurpar la seleccion
...
function enviarSticker(stickerFile, message) {
    ...
    try {
        ...
        // Enviar el sticker como mensaje independiente
...
function manejarBotMenu(message) {
    ...
    // Limpiar cualquier solicitud previa del usuario antes de asignar una nueva
    solicitudesCategorias.delete(key);
    solicitudesStickersPorCategoria.delete(key);
    solicitudesAudiosCategorias.delete(key);
    solicitudesMenuPrincipal.delete(key); // Asegurar que no haya duplicados antes
    ...
    // Registrar que este usuario está en el menú
    solicitudesMenuPrincipal.set(key, true);

    // Eliminar el estado después de 60 segundos
    setTimeout(() => {
