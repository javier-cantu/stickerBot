// sb007_coincidenciasExactas.js
// Este responde la lista de stickers con keywords pero ahora si con coincidencias exactas. 
...
function buscarStickersPorPalabrasClave(palabrasClave) {
    const palabras = palabrasClave.toLowerCase().trim().split(/\s+/); // Manejar espacios múltiples y normalizar
    const resultados = [];

    // Buscar en el stickersData
    for (const [stickerFile, tags] of Object.entries(stickersData)) {
        if (tags.some(tag => palabras.includes(tag.toLowerCase()))) { // Comparar con coincidencias exactas
