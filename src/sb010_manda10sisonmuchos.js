// sb010_manda10sisonmuchos.js
// quiero que mande solo 10 stickers si son mas de 10 los que hay con los resultados. 
// Y quiero que haya 1 segundo entre cada envio. 
        ...
        if (comando.startsWith('quiero stickers de')) {
            ...
            if (palabrasClave) {
                ...
                if (resultados.length > 0) {
                    const limit = 10; // Limitar a los primeros 10 resultados
                    const resultadosLimitados = resultados.slice(0, limit);

                    message.reply(`Encontré los siguientes stickers (máximo ${limit}):\n${resultadosLimitados.join('\n')}`);

                    // Enviar stickers uno por uno con un retraso de 1 segundo entre cada envío
                    for (const stickerFile of resultadosLimitados) {
...
function buscarStickersPorPalabrasClave(palabrasClave) {
    ...
    for (const [stickerFile, tags] of Object.entries(stickersData)) {
        // Verificar si todas las palabras clave están en las etiquetas
        if (palabras.every(palabra => tags.map(tag => tag.toLowerCase()).includes(palabra))) {
